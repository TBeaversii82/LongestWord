...
function bookStats(name) {
    //alert(name);
    let bookArray = [];
    longestWord = "";

    if (name == 'alice') {
         let tempbook1 = book1.replace("?", '');
        bookArray = tempbook1.split(' ');
    }
    else if (name == 'harry') {
        let tempbook2 = book2.replace("?", '').replace(",", '');
        bookArray = tempbook2.split(' ');
    }
    else {
        let tempbook3 = book3.replace("?", '').replace(",", '');
        bookArray = tempbook3.split(' ');
    }

    for (i = 0; i < bookArray.length; i++) {
        let tempWord = bookArray[i];
        if (tempWord.length > longestWord.length) {
            longestWord = tempWord;
        }
    }
    document.getElementById("bookoutput").innerHTML = `The longest word is: ${longestWord} | The word count is ${bookArray.length}`;
}